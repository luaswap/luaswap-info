...
function PairList({ pairs, color, disbaleLinks, maxItems = 10 }) {
  ...
  const ListItem = ({ pairAddress, index }) => {
    ...
    if (pairData && pairData.token0 && pairData.token1) {
      ...
      const apy = formattedPercent((pairData.oneDayVolumeUSD * FEE_SWAP * 365 * 100) / pairData.reserveUSD)
      ...
      return (
        <DashGrid style={{ height: '48px' }} disbaleLinks={disbaleLinks} focus={true}>
          ...
          {!below1080 && <DataText area="fees">{formattedNum(pairData.oneDayVolumeUSD * FEE_SWAP, true)}</DataText>}
    ...
    Object.keys(pairs)
      .sort((addressA, addressB) => {
        ...
        if (sortedColumn === SORT_FIELD.APY) {
          const apy0 = parseFloat(pairA.oneDayVolumeUSD * FEE_SWAP * 356 * 100) / parseFloat(pairA.reserveUSD)
          const apy1 = parseFloat(pairB.oneDayVolumeUSD * FEE_SWAP * 356 * 100) / parseFloat(pairB.reserveUSD)
